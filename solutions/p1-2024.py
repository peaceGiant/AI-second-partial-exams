...
def modify_dataset(dataset):
    new_dataset = [[row[0] + row[10]] + [el for ind, el in enumerate(row) if ind not in [0, 10]] for row in dataset]
    return new_dataset
...
def split_dataset(dataset, C, P):
    ...
    if C == 0:
        ...
        train_set = dataset_good[int((100 - P) / 100 * len(dataset_good)):] + dataset_bad[int((100 - P) / 100 * len(dataset_bad)):]
        test_set = dataset_good[:int((100 - P) / 100 * len(dataset_good))] + dataset_bad[:int((100 - P) / 100 * len(dataset_bad))]
        train_X, train_Y = [row[:-1] for row in train_set], [row[-1] for row in train_set]
        test_X, test_Y = [row[:-1] for row in test_set], [row[-1] for row in test_set]
